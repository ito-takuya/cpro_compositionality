# BIG change 3/5/21 - changed stim input size = 20 element array (including boolean pairs for logic rule training)
...
def create_random_trials(taskRuleSet,ntrials_per_task,seed):
    ...
    stim_ind = np.arange(11,31) # stimuli are the last 16 indices of input vector
...
def create_taskcontext_inputsOnly(taskRuleSet,output_taskinfo=False):
    ...
    # Construct trial dynamics
    rule_ind = np.arange(11) # rules are the first 11 indices of input vector
    stim_ind = np.arange(11,31) # stimuli are the last 20 (formerly 16) indices of input vector
    input_size = len(rule_ind) + len(stim_ind)
                        ...
                        for pit2 in pitch:
                            for con1 in constant:
                                for con2 in constant:
                                    code = np.zeros((8*nStims+4,)) # + 4 boolean stim
                                    # Stim 1
                                    code[col1] = 1
...
def _create_sensorytask_stimuli():
    ...
    stim_ind = np.arange(20) # 16 + 4 boolean stim total stimulus combinations
...
def _create_pretraining_stimuli():
    ...
    stim_ind = np.arange(20) # 16 total stimulus combinations
