...
basedir = '/projects3/CPROCompositionality/'
stimdir = basedir + 'data/stimfiles/'


def loadBetaSeries(subj):
    ...
    for mb in range(1,n_miniblocks+1): 
        probe = glob.glob(stimdir + subj + '*Miniblock' + str(mb) + '_Probe1.1D')
        probe = np.loadtxt(probe[0])
        regressors.append(probe)

        probe = glob.glob(stimdir + subj + '*Miniblock' + str(mb) + '_Probe2.1D')
        probe = np.loadtxt(probe[0])
        regressors.append(probe)

        probe = glob.glob(stimdir + subj + '*Miniblock' + str(mb) + '_Probe3.1D')
        probe = np.loadtxt(probe[0])
        regressors.append(probe)
