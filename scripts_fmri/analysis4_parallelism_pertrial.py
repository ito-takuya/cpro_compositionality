...
def run(args):
    ...
    if logic:
        ...
        ps_mat = np.zeros((360,64,len(subjNums)))
        scount = 0
        for subj in subjNums:
            df = {}
            df['ROI'] = []
            ...
            df['Rule'] = []
            df['Parallelism'] = []
            print('Computing PS for subject', subj)
            inputs = []
            for roi in rois:
            ...
            results = pool.starmap_async(tools.parallelismScoreTrial,inputs).get()
            pool.close()
            pool.join()
                        
            i = 0
            for roi in rois:
                ps_mat[i,:,scount] = results[i]
                i += 1
            
            scount += 1

        h5f = h5py.File(resultdir + 'ParallelismScoreTrialwise/LogicRules.h5','a')
        try:
            h5f.create_dataset('data',data=ps_mat)
        except:
            del h5f['data']
            h5f.create_dataset('data',data=ps_mat)
        h5f.close()
    ...
    if sensory:
        ...
        for subj in subjNums:
            ...
            for roi in rois:
                roi_ind = np.where(glasser==roi)[0]
                mat = data_mat[subj][roi_ind,:].T
                inputs.append((mat,sensory_labels[subj],logic_labels[subj],motor_labels[subj],permutation))
        ...
        h5f = h5py.File(resultdir + 'ParallelismScoreTrialwise/SensoryRules.h5','a')
        try:
            h5f.create_dataset('data',data=ps_mat)
        except:
            del h5f['data']
    ...
    if motor:
        ...
        for subj in subjNums:
            ...
            for roi in rois:
                roi_ind = np.where(glasser==roi)[0]
                mat = data_mat[subj][roi_ind,:].T
                inputs.append((mat,motor_labels[subj],sensory_labels[subj],logic_labels[subj],permutation))
        ...
        h5f = h5py.File(resultdir + 'ParallelismScoreTrialwise/MotorRules.h5','a')
        try:
            h5f.create_dataset('data',data=ps_mat)
        except:
            del h5f['data']
