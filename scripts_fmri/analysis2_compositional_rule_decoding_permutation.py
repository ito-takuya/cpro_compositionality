...
def run(args):
    ...
    if logic:
        ...
        nulldist = np.zeros((len(rois),npermutations))
        roicount = 0
        for roi in rois:
            print('Running logic rule decoding permutations... ROI:', roi)
            roi_ind = np.where(glasser==roi)[0]
            roi_data = data_mat[:,roi_ind]
            inputs = []
            for i in range(npermutations):
                inputs.append((roi_data,subj_labels,task_labels,kfold,normalize,classifier,confusion,permutation,np.random.randint(10000000)))
            ...
            nulldist[roicount,:] = accuracies
            roicount += 1


        np.savetxt(resultdir + 'CrossConditionGeneralization/LogicDecoding_NullDistribution.csv',nulldist)
    ...
    if sensory:
        ...
        roicount = 0
        for roi in rois:
            print('Running sensory rule decoding permutations... ROI:', roi)
            roi_ind = np.where(glasser==roi)[0]
            roi_data = data_mat[:,roi_ind]
            inputs = []
            for i in range(npermutations):
        ...
        np.savetxt(resultdir + 'CrossConditionGeneralization/SensoryDecoding_NullDistribution.csv',nulldist)
    ...
    if motor:
        ...
        roicount = 0
        for roi in rois:
            print('Running motor rule decoding permutations... ROI:', roi)
            roi_ind = np.where(glasser==roi)[0]
            roi_data = data_mat[:,roi_ind]
            inputs = []
            for i in range(npermutations):
        ...
        np.savetxt(resultdir + 'CrossConditionGeneralization/MotorDecoding_NullDistribution.csv',nulldist)
